...
class ShareLink extends AppModel {
	...
	private function _addHashedPassword($data) {
		if (!empty($data['ShareLink']['hashed_password'])) {
			...
			$data['ShareLink']['hashed_password'] = $this->Auth->password($data['ShareLink']['hashed_password']);
		}
		return $data;
	}
	...
	* */
	...
	function getAllForOwner($onwerId, $onlyWithRenewalRequest = false) {
		$params = array('conditions' => array('ShareLink.owner_id' => $onwerId));
		if ($onlyWithRenewalRequest) {
